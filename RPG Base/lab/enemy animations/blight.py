...
attack_reach = 60
attack_curve = [5, 1,7,3] # windup, approach, impact, recede
...
while True:
    ...
    if not attack:
        ...
        angle = math.radians(angle_to(left_hand_pos, attack_target))
        dist = distance(left_hand_pos, attack_target)
        if attack_timer >= attack_timer_max-attack_curve[0]: # in the approach phase
            windup = 30
            left_hand_pos[0] -= math.cos(angle)*windup/attack_curve[0]
            left_hand_pos[1] -= math.sin(angle)*windup/attack_curve[0]

        elif attack_timer >= attack_timer_max-sum(attack_curve[:2]): # in the approach phase
            left_hand_pos[0] += math.cos(angle)*attack_reach/attack_curve[1]
            left_hand_pos[1] += math.sin(angle)*attack_reach/attack_curve[1]

        elif attack_timer >= attack_timer_max-sum(attack_curve[:3]): # in the approach phase
            pass

        else: # in the approach phase
            approach = [razak_rect.centerx - 70, razak_rect.centery + math.cos(t)*4 + 30]
            angle = math.radians(angle_to(left_hand_pos, approach))
            dist = distance(left_hand_pos, approach)
            left_hand_pos[0] += math.cos(angle)*dist/attack_curve[3]
            left_hand_pos[1] += math.sin(angle)*dist/attack_curve[3]
