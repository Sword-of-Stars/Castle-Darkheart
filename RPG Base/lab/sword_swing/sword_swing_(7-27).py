...
class Sword():
    def __init__(self, player):
        ...
        self.pos = self.set_pos(0)
        self.smear_pos = self.set_pos(90)
    ...
    def load_smear_images(self):
        ...
        self.smears = [pygame.transform.scale_by(x, (3,2)) for x in self.smears]
    ...
    def set_angle(self, pos):
        ...
        if not self.is_swing:
            ...
            self.set_pos(self.angle_to_player+self.angular_velocity*self.swing_step*self.swing_dir)
            
        self.smear_pos = (self.player.pos[0]+self.length*math.cos(math.radians(angle)), 
                        self.player.pos[1]+self.length*math.sin(math.radians(angle)))

        self.angle = -angle + self.initial_angle + 90
        self.smear_angle = -angle + 180
        self.angle_to_player = a

    def set_pos(self, angle):
        self.pos = (self.player.pos[0]-self.length*math.cos(math.radians(angle)), 
                    self.player.pos[1]-self.length*math.sin(math.radians(angle)))
    ...
    def handle_swing(self):
        ...
        self.angle += self.angular_velocity*self.swing_step*self.swing_dir
